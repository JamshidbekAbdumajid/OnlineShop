...
def dashboard(request):
    if request.user.is_admin:
        # Admin uchun maxsus sahifa
        return render(request, 'store/admin_dashboard.html')
    else:
        # Oddiy mijozlarni asosiy sahifaga qaytarish
        return redirect('main')

@login_required
def add_product(request):
    if not request.user.is_admin:  # Faqat admin foydalanuvchilar uchun mavjud
        return HttpResponseForbidden("Sizda bu amalni bajarish uchun yetarli huquq mavjud emas.")

    if request.method == "POST":
        form = AddProductForm(request.POST, request.FILES)
        if form.is_valid():
            form.save()
            return redirect('dashboard')  # Admin boshqaruv paneliga qaytish
    else:
        form = AddProductForm()
...
def edit_product(request, product_id):
    ...
    if not request.user.is_admin:
        return HttpResponseForbidden("Bu amal faqat adminlar uchun mavjud.")
